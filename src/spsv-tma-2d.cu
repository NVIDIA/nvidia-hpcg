...
/*!
 ...
 TMA2D SymGS triangular solve: the per-colour launch of the other solves, with
 the matrix stream staged through shared memory by a tensor map rather than by
 addresses the kernel computes.

 The Sliced-ELL columns and values of one slice are a dense
 slice_size-by-max_row_len tile, so the whole triangle is a two-dimensional
 ...
 The copy engine then moves a whole UNROLL-deep box per instruction, and reads
 past the slice's last stored entry are zero-filled by the hardware instead of
 being skipped by the issuing lane.
 ...
 the block is required to sit inside a single slice, so its coordinates resolve
 once, in one elected lane, and are broadcast through shared memory. Two shared
 buffers are double-buffered against each other over an mbarrier pair, as in the
 1-D family.
 ...
 change across it. Unlike the 1-D family, a thread owns RPT *consecutive* rows
 rather than RPT rows a block size apart, so those reads and the x store are
...
__global__ __launch_bounds__(BLKDIM) void spsv_sell_tma2d(int color_str, int color_end, double* __restrict__ x,
    const double* __restrict__ rhs, const double* __restrict__ diag, const OffsetT* __restrict__ slice_offsets,
    ...
    const __grid_constant__ CUtensorMap col_map)
{
    ...
    if (IsWarpZero() && IsElectedLane())
    {
        ...
        // A slice offset is bounded by the triangle's padded nonzero count, and
...
#pragma unroll
    ...
    // No liveness mask on these reads and no row guard on the store, unlike the
    // 1-D family: the launcher requires color_size % ROWS == 0, so a CTA's rows
    // all belong to its colour and the three accesses are one vector each.
    // color_end is what the 1-D kernel guards with and is kept in the signature
    // so the two read alike from the call site.
...
template <class OffsetT, int BLKDIM, int UNROLL, int RPT>
bool LaunchSpsvTma2d(bool forward, const SparseMatrix& A, const double* rv, double* xv, const OffsetT* slice_offsets,
    const idx32_t* columns, const double* values, slice_ptr_t last_nnz, cudaStream_t stream)
{
    constexpr int ROWS = BLKDIM * RPT;
    constexpr int TROWS = (ROWS <= 256) ? ROWS : 256;
    constexpr int TILE = UNROLL * ROWS;
    ...
    // The CTA's whole row block has to lie inside one slice and inside one
    // colour: the kernel resolves its two tile coordinates once, from its first
    // row, and every box it then fetches is measured from them. Together with a
    // padded nonzero count that is a whole number of slices, these are also what
    // makes the tensor description legal -- the map's global stride is
    // slice_size elements, so a trailing partial slice would put rows of one
    // slice inside another's tile, and a box origin that is not a multiple of
    // the box extent would straddle two tiles.
    ...
    // range and refuses only a triangle whose tiles genuinely cannot be
    // addressed.
    ...
    auto kernel = spsv_sell_tma2d<OffsetT, BLKDIM, UNROLL, RPT>;
...
// family's 16 because a thread's rows are consecutive and are loaded and stored
// as one vector access, and 4 doubles is the widest one the hardware has; the
// block size stops at 256 for the same reason the SpMV's does, since the row
// block is what the box covers rather than what the colour allows.
//
// last_nnz is the triangle's padded nonzero count, which the 1-D family does not
...
bool SpsvTma2dSellCfg(bool forward, const SparseMatrix& A, const double* rv, double* xv, const OffsetT* slice_offsets,
    ...
    int rpt)
{
#define HPCG_SV_TMA2D_CASE(B, U, R)                                                                                    \
    if (blk == (B) && unroll == (U) && rpt == (R))                                                                     \
        return LaunchSpsvTma2d<OffsetT, B, U, R>(                                                                      \
            forward, A, rv, xv, slice_offsets, columns, values, last_nnz, stream);
#define HPCG_SV_TMA2D_ROW(B, U) HPCG_SV_TMA2D_CASE(B, U, 1) HPCG_SV_TMA2D_CASE(B, U, 2) HPCG_SV_TMA2D_CASE(B, U, 4)
#define HPCG_SV_TMA2D_BLK(B)                                                                                           \
    HPCG_SV_TMA2D_ROW(B, 1)                                                                                            \
    HPCG_SV_TMA2D_ROW(B, 2)                                                                                            \
    HPCG_SV_TMA2D_ROW(B, 3) HPCG_SV_TMA2D_ROW(B, 4) HPCG_SV_TMA2D_ROW(B, 6) HPCG_SV_TMA2D_ROW(B, 8)
    HPCG_SV_TMA2D_BLK(32)
    HPCG_SV_TMA2D_BLK(64)
    HPCG_SV_TMA2D_BLK(128)
    HPCG_SV_TMA2D_BLK(256)
#undef HPCG_SV_TMA2D_BLK
#undef HPCG_SV_TMA2D_ROW
#undef HPCG_SV_TMA2D_CASE
    return false;
}

// Both slice-offset widths, 32-bit columns: --mi 0 and --mi 1 respectively.
template bool SpsvTma2dSellCfg<idx32_t>(bool, const SparseMatrix&, const double*, double*, const idx32_t*,
    const idx32_t*, const double*, slice_ptr_t, cudaStream_t, int, int, int);
template bool SpsvTma2dSellCfg<idx64_t>(bool, const SparseMatrix&, const double*, double*, const idx64_t*,
    const idx32_t*, const double*, slice_ptr_t, cudaStream_t, int, int, int);
