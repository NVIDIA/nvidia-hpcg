...
__global__ __launch_bounds__(BLKDIM) void mv_sell_tma2d(int m, double alpha, double beta, double* __restrict__ y,
    const double* __restrict__ x, const slice_ptr_t* __restrict__ slice_offsets, int slice_size,
    ...
    slice_ptr_t* slice_offsets, local_int_t* columns, double* values, slice_ptr_t last_nnz, cudaStream_t stream)
{
    ...
    auto kernel = mv_sell_tma2d<BLKDIM, UNROLL, RPT>;
...
#define TRY(B, U, R)                                                                                                   \
    if (blk == (B) && unroll == (U) && rpt == (R))                                                                     \
        return LaunchMvTma2d<B, U, R>(                                                                                 \
            A, alpha, beta, x, y, sell_block_offset, sell_columns, sell_values, last_nnz, stream);
...
bool MvTma2dSellCfg(const SparseMatrix&, double, double, const double*, double*, slice_ptr_t*, local_int_t*,
    double*, slice_ptr_t, cudaStream_t, int, int, int)
{
    return false;
}
