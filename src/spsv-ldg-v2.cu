    ...
    double alpha, local_int_t slice_size, intdiv32_t slice_size_div)
{
    ...
    // They sit on the per-thread critical path, so both go through the same
    // precomputed magic-number reciprocal rather than a hardware divide.
    int slice, in_slice;
    ...
    const idx32_t* columns, const double* values, cudaStream_t stream)
{
    ...
    // Generated once on the host and reused by every colour launch below.
    ...
    for (local_int_t i = 0; i < A.totalColors; ++i)
    {
        ...
        spsv_sell_ldgv2<OffsetT, BLKDIM, UNROLL, W><<<grid, BLKDIM, 0, stream>>>(color_str, color_str + color_size, xv,
            rv, slice_offsets, columns, values, A.diagonal, alpha, slice_size, sdiv);
    }
    return true;
}
