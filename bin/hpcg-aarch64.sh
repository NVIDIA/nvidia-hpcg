...
usage() {
  ...
  echo "    --exm <int>                   specifies the execution mode. 0 is GPU-only, 1 is CPU(aarch64)-only, and 2 is GPU+CPU(aarch64). default is 0"
  ...
  echo "    --ddm <int>   specifies the direction that GPU and CPU will not have the same local dimension. 0 is auto, 1 is x, 2 is y, and 3 is z. Default is 0"
  echo "    --lpm <int>   controls the meaning of the value provided for --g2c parameter. Applicable when --exm is 2"
  echo "                  - 0 means nx/ny/nz are GPU dims and g2c is the ratio of GPU dim to CPU dim. e.g., --g2c 8 means the different CPU dim is 1/8 the GPU dim"
  echo "                  - 1 means nx/ny/nz are GPU dims and g2c is the abs value for the different dim for CPU. e.g., --g2c 64 means the different CPU dim is 64"
  echo "                  - 2 assumes the sum of different dims of GPU and CPU is part of nx, ny, or nz (depend on --ddm) and --g2c is the ratio. e.g., --ddm 1, --nx 1024, and --g2c 8 then GPU nx is 896 and CPU nx is 128"
  echo "                  - 3 assumes the sum of different dims of GPU and CPU is part of nx, ny, or nz (depend on --ddm) and --g2c is absolute. e.g., --ddm 1, --nx 1024, and --g2c 96 then GPU nx is 928 and CPU nx is 96"
  echo "    --g2c   <int> specifies the differnt dimensions of the GPU and CPU ranks. Depends on --lpm value"
